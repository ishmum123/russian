#!/bin/sh
# Runs every check that must pass before shipping the Russian pack/site:
#   1. packbuilder check         - pack-level schema/coverage/article checks
#      (engine/tools/packbuilder/qa/check.py with the Russian spec)
...
PYTHONPATH="${PACKBUILDER_PATH:-engine/tools}" python3 -m packbuilder check --lang ru --repo .
...
TMP="$(mktemp /tmp/russian_index_check.XXXXXX.html)"
trap 'rm -f "$TMP"' EXIT
